...
export async function createCalendarEvent({
  ...
  if (calendarUser.primaryEmailAddress == null) {
    throw new Error(
      "El usuario no tiene una dirección de correo electrónico principal"
    );
  }
  ...
  const calendarEvent = await google.calendar("v3").events.insert({
    ...
    requestBody: {
      ...
      description: guestNotes ? `Notas: ${guestNotes}` : undefined,
      start: {
...
async function getOAuthClient(clerkUserId: string) {
  ...
  if (token.data.length === 0 || token.data[0].token === null)
    throw new Error("No se encontró el token de OAuth de Google");
